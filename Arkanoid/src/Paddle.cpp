...
void Paddle::getSinCos(fix16 ang, fix16& outSin, fix16& outCos) const {
    // NA STM32: Tutaj użyj tablicy lookup (LUT) lub arm_sin_f32 dla wydajności!
...
void Paddle::update(const InputState& input, fix16 worldW, fix16 worldH) {
    ...
    fix16 safeX = clamp(input.normX, FIX(0), FIX(1));
    fix16 safeY = clamp(input.normY, FIX(0), FIX(1));

    pos.x = MUL(safeX, worldW);
    pos.y = MUL(safeY, worldH);
